...
class EchoStateNetwork:
    ...
    def __init__(
        ...
        self._model_name: str = hyper_parameter.get("modelName") or "ESN_model"
    ...
    def visualize_fit(
        ...
        if scaler is not None:
            y_train = scaler.inverse_transform(np.append(x_train, y_train, axis=1))[
                :, -y_train.shape[1] :
            ]
            y_hat = scaler.inverse_transform(
                np.append(x_train[self._no_forget_points :], y_hat, axis=1)
            )[:, -y_hat.shape[1] :]
    ...
    def moving_window_forecasts(
        self, data, index, forecast_horizon=30, window_mode="Expanding", window_size=20
    ):
        """Refit the ESN with past data and forecast from the most recent point of the known data.
        :param data: Data Object
        :param index: int, the index that marks the the most recent point of the 'known' data
        :param forecast_horizon: int, time steps that the iterative forecast will reach into the future
...
def __evaluate_esn_model(
    data: dataUtils.Data,
    internal_nodes: int,
    spectral_radius: float,
    regression_lambda: float,
    connectivity: float,
    leaking_rate: float,
) -> float:
    """
    ...
    test_esn = EchoStateNetwork(
        no_input_nodes=data.noTimeSeries,
        no_output_nodes=data.noTimeSeries,
...
def hyper_parameter_grid_search(
    data: dataUtils.Data, split_index: int, hyperparameter_space: dict = None
):
    """Perform hyperparamter grid search optimization
    :param data: Data object
    :param split_index: split the data into training and test set
    :param hyperparameter_space: a dictionary containing the limitis of the hyper parameter search
    ...
    if hyperparameter_space is None:
        hyperparameter_space = {
            "internal_nodes": (30, 400, 10),
            "spectral_radius": (0.1, 1.2, 10),
            "regression_lambda": (0.001, 1e-10, 10),
            "connectivity": (0.01, 0.5, 10),
            "leaking_rate": (0, 0.3, 10),
        }

    search_points = dict()
    for param in hyperparameter_space.keys():
        p_start = hyperparameter_space[param][0]
        p_stop = hyperparameter_space[param][1]
        p_num = hyperparameter_space[param][3]
    ...
    for combination in parameter_combinations:
        ...
        if np.abs(test_error) < min_error:
            ...
            optimal_hyperparameter = hyper_parameter_sample

    print("Grid Search Completed. Min Error %f" % min_error)
    pprint(optimal_hyperparameter)

    return min_error, optimal_hyperparameter
...
def hyper_parameter_bayesian_optimization(
    ...
    if hyperparameter_space is None:
        hyperparameter_space = {
            "internal_nodes": (30, 400),
            "spectral_radius": (0.1, 1.2),
            "regression_lambda": (0.001, 1e-10),
            "connectivity": (0.01, 0.5),
            "leaking_rate": (0, 0.3),
        }

    optimizer = BayesianOptimization(
        f=__evaluate_esn_model, pbounds=hyperparameter_space, random_state=1
    )
